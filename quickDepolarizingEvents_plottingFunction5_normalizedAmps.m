function [] = quickDepolarizingEvents_plottingFunction5_normalizedAmps(Vtraces_columns,time_axis,VpeaksIdcs_cellArray,baseline_Vs_cellArray)

plotWindow_startidx = -150;
plotWindow_endidx = 600;
windowed_time_axis = time_axis(1:751);

    QDEamps_cellArray = cell(length(VpeaksIdcs_cellArray),1);
    ...
    for i = 1:length(VpeaksIdcs_cellArray)
        ...
        QDEamps_cellArray{i} = Vtraces_columns(VpeaksIdcs_cellArray{i},i) - baseline_Vs_cellArray{i};
...
for i = 1:length(VpeaksIdcs_cellArray)
    if ~isempty(VpeaksIdcs_cellArray{i})
        ...
        tracei_QDEamps = QDEamps_cellArray{i};
        ...
        for j = 1:length(tracei_VpeaksIdcs)
            ...
            QDEj_Vtrace_baselined_normalized = QDEj_Vtrace_baselined / tracei_QDEamps(j);
            plot(windowed_time_axis,QDEj_Vtrace_baselined_normalized,'color',colormap(tracei_QDE_colorNos(j),:));
        end
    end
end
bar = colorbar;
