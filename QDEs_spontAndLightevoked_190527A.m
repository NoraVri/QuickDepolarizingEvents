...
collectedData.voltage = Vs;
collectedData.smoothedVoltage = Vs_smoothed;
collectedData.current = Is;
collectedData.TTL = TTLs;
collectedData.time_axis = time_axis;
...
%which detects fast events based on Vderivative and filters them based on amplitude, baselineV stability and decay back towards baseline
no_of_traces = length(collectedData.voltage(1,:));
Vpeaks_idcs = cell(1,no_of_traces);
baselineVs = cell(1,no_of_traces);

min_QDEamp = 2;
for i = 1:no_of_traces
[Vpeaks_idcs{i},baselineVs{i}] = finding_fastDepolarizingPotentials(collectedData.voltage(:,i),min_QDEamp);
%plotting each Vtrace and corresponding TTL pulse, marking peaks and baselines on detected QDEs
figure;hold on;
    plot(collectedData.time_axis,collectedData.voltage(:,i),'b');
    plot(collectedData.time_axis,smoothdata(collectedData.voltage(:,i),'movmedian',10),'k');
    plot(collectedData.time_axis,collectedData.TTL(:,i)+mean(collectedData.voltage(:,i)),'r');
    scatter(collectedData.time_axis(Vpeaks_idcs{i}),collectedData.voltage(Vpeaks_idcs{i},i),'r','filled');
    scatter(collectedData.time_axis(Vpeaks_idcs{i} - 100),baselineVs{i},'g','filled');
%filtering out spikes idcs
    peakVs_i = collectedData.voltage(Vpeaks_idcs{i},i);
    Vpeaks_idcs{i}(peakVs_i > 0) = [];
    baselineVs{i}(peakVs_i > 0) = [];
end
%adding results into collectedData
collectedData.Vpeaks_idcs = Vpeaks_idcs;
collectedData.baselineVs = baselineVs;

%% 3b: getting amp, rise-time and half-width for all "clean" QDEs
for i = 120:123
[amps,riseTimes,halfWidths] = getQDEmeasures(collectedData.voltage(:,i),collectedData.Vpeaks_idcs{i},collectedData.baselineVs{i});
end
...
for i = 1:no_of_traces
    tracei_TTLon = find(collectedData.TTL(:,i)>9);
...
%adding results into collectedData
collectedData.spontQDEs_idcs = spontQDEs_peaksIdcs;
collectedData.spontQDEs_baselineVs = spontQDEs_baselineVs;
collectedData.evokedQDEs_idcs = evokedQDEs_peaksIdcs;
collectedData.evokedQDEs_baselineVs = evokedQDEs_baselineVs;
...
%adding results into collectedData
collectedData.lowBaselineQDEs_peaksIdcs = lowBaselineQDEs_peaksIdcs;
collectedData.lowBaselineQDEs_baselineVs = lowBaselineQDEs_baselineVs;
collectedData.highBaselineQDEs_peaksIdcs = highBaselineQDEs_peaksIdcs;
collectedData.highBaselineQDEs_baselineVs = highBaselineQDEs_baselineVs;
%% step5: plotting 
%low-baseline spont
...
quickDepolarizingEvents_plottingFunction3(collectedData.voltage,collectedData.time_axis,lowBaseline_spontQDEs_peaksIdcs,lowBaseline_spontQDEs_baselineVs);
...
quickDepolarizingEvents_plottingFunction4(collectedData.voltage,collectedData.time_axis,lowBaseline_evokedQDEs_peaksIdcs,lowBaseline_evokedQDEs_baselineVs);
...
quickDepolarizingEvents_plottingFunction3(collectedData.voltage,collectedData.time_axis,highBaseline_spontQDEs_peaksIdcs,highBaseline_spontQDEs_baselineVs);
...
quickDepolarizingEvents_plottingFunction4(collectedData.voltage,collectedData.time_axis,highBaseline_evokedQDEs_peaksIdcs,highBaseline_evokedQDEs_baselineVs);
...
lowBaseline_spontQDEs_peaksIdcs = intersecting_vectors_in_cells(lowBaselineQDEs_peaksIdcs,spontQDEs_peaksIdcs);
lowBaseline_spontQDEs_baselineVs = intersecting_vectors_in_cells(lowBaselineQDEs_baselineVs,spontQDEs_baselineVs);
quickDepolarizingEvents_plottingFunction5_normalizedAmps(collectedData.voltage,collectedData.time_axis,lowBaseline_spontQDEs_peaksIdcs,lowBaseline_spontQDEs_baselineVs);
%overlay low-baseline evoked
lowBaseline_evokedQDEs_peaksIdcs = intersecting_vectors_in_cells(lowBaselineQDEs_peaksIdcs,evokedQDEs_peaksIdcs);
lowBaseline_evokedQDEs_baselineVs = intersecting_vectors_in_cells(lowBaselineQDEs_baselineVs,evokedQDEs_baselineVs);
quickDepolarizingEvents_plottingFunction6_normalizedAmps(collectedData.voltage,collectedData.time_axis,lowBaseline_evokedQDEs_peaksIdcs,lowBaseline_evokedQDEs_baselineVs);
title('low-baseline QDEs, amp normalized, black = light-evoked')

%high-baseline spont
highBaseline_spontQDEs_peaksIdcs = intersecting_vectors_in_cells(highBaselineQDEs_peaksIdcs,spontQDEs_peaksIdcs);
highBaseline_spontQDEs_baselineVs = intersecting_vectors_in_cells(highBaselineQDEs_baselineVs,spontQDEs_baselineVs);
quickDepolarizingEvents_plottingFunction5_normalizedAmps(collectedData.voltage,collectedData.time_axis,highBaseline_spontQDEs_peaksIdcs,highBaseline_spontQDEs_baselineVs);
%overlay high-baseline evoked
highBaseline_evokedQDEs_peaksIdcs = intersecting_vectors_in_cells(highBaselineQDEs_peaksIdcs,evokedQDEs_peaksIdcs);
highBaseline_evokedQDEs_baselineVs = intersecting_vectors_in_cells(highBaselineQDEs_baselineVs,evokedQDEs_baselineVs);
quickDepolarizingEvents_plottingFunction6_normalizedAmps(collectedData.voltage,collectedData.time_axis,highBaseline_evokedQDEs_peaksIdcs,highBaseline_evokedQDEs_baselineVs);
title('high-baseline QDEs, amp normalized, black = light-evoked')

%%
quickDepolarizingEvents_plottingFunction3(collectedData.voltage,collectedData.time_axis,spont_VpeaksIdcs_lowBaseline,spont_baselineVsIdcs_lowBaseline);
ylim([-1 9])
title('spontaneously occurring quick depolarizing events; baseline <-65')
quickDepolarizingEvents_plottingFunction3(collectedData.voltage,spontData.time_axis,spont_VpeaksIdcs_highBaseline,spont_baselineVsIdcs_highBaseline);
ylim([-1 9])
title('spontaneously occurring quick depolarizing events; baseline >-60')

quickDepolarizingEvents_plottingFunction3(collectedData.voltage,collectedData.time_axis,evoked_VpeaksIdcs_lowBaseline,evoked_baselineVsIdcs_lowBaseline);
ylim([-1 9])
title('quick depolarizing events evoked by light; baseline <-65')
quickDepolarizingEvents_plottingFunction3(collectedData.voltage,collectedData.time_axis,evoked_VpeaksIdcs_highBaseline,evoked_baselineVsIdcs_highBaseline);
ylim([-1 9])
