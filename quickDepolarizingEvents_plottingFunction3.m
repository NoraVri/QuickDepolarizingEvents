function [] = quickDepolarizingEvents_plottingFunction3(Vtraces_columns,time_axis,VpeaksIdcs_cellArray,baseline_Vs_cellArray)
...
for i = 1:length(VpeaksIdcs_cellArray)
    if ~isempty(VpeaksIdcs_cellArray{i})
        ...
        for j = 1:length(tracei_VpeaksIdcs)
            ...
            plot(windowed_time_axis,QDEj_Vtrace_baselined,'color',colormap(tracei_QDE_colorNos(j),:));
        end
    end
end
bar = colorbar;
